    ...
    private fun observeViewModel() {
        viewModel.authState.observe(this) { state ->
            when (state) {
                ...
                is AuthState.CodeSent     -> { setInProgress(false); AndroidUtil.showToast(this, "OTP enviado com sucesso") }
                is AuthState.SignInSuccess -> {
                    setInProgress(false)
                    startActivity(Intent(this, LoginUsernameActivity::class.java).apply {
                        putExtra("phone", phoneNumber)
                    })
                }
                is AuthState.Error        -> { setInProgress(false); AndroidUtil.showToast(this, state.message) }
